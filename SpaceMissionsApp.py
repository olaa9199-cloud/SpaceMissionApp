...
<head>
    ...
    <style>
        ...
        /* Styles for when the date input is faded */
        ...
        /* Styles for when the date input is normal (not faded) */
...
<body>
    ...
    <script>
        ...
        function updateComponentUI(args) {
            ...
            const { initial_year, initial_month, initial_day, max_year, is_faded_on_invalid, has_missions_for_birthday_status } = args;
            ...
            // This is the updated logic to control fading based on validity and missions
            if (is_faded_on_invalid) {
                root.classList.add('faded');
                root.classList.remove('normal');
            } else if (has_missions_for_birthday_status === true) {
...
def birthday_input_component(initial_year=2000, initial_month=1, initial_day=1, has_missions_for_birthday_status=None, key=None):
    ...
    """
    is_faded_on_invalid = False
    try:
        datetime.date(initial_year, initial_month, initial_day)
    except ValueError:
        is_faded_on_invalid = True
    ...
    return _my_birthday_input(
        ...
        is_faded_on_invalid=is_faded_on_invalid,
...
if 'bday_component_year' not in st.session_state:
    st.session_state.bday_component_year = datetime.datetime.now().year
    st.session_state.bday_component_month = datetime.datetime.now().month
    st.session_state.bday_component_day = datetime.datetime.now().day
